...
def yolo_non_max_suppression(scores, boxes, classes, max_boxes = 10, iou_threshold = 0.5):
    ...
    _boxes_tensor = tf.Variable(max_boxes, dtype='int32')  # tensor to be used in tf.image.non_max_suppression()
    #K.get_session().run(tf.variables_initializer([max_boxes_tensor]))  # initialize variable max_boxes_tensor

    # Use tf.image.non_max_suppression() to get the list of indices corresponding to boxes you keep
    nms_indices = tf.image.non_max_suppression(boxes, scores, _boxes_tensor, iou_threshold=iou_threshold)
...
def predict(image_file):
    ...
    #draw_boxes(image, out_boxes, out_classes, classmethod, out_scores)
    draw_boxes(image, out_scores, out_boxes, out_classes, class_names, colors)
    # Save the predicted bounding box on the image
    # Display the results in the notebook
    image.save(os.path.join("images", "out", image_file))
    imshow(image)
